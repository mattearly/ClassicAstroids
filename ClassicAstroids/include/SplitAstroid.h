...
void splitAstroid(Astroid& a, std::vector<Astroid>& astriods)
{
	...
	AA::GameObject& our_ref = AA::LOOP->getGameObject(a.object_id);
	glm::vec3 original_loc = our_ref.getLocation();

	// turn the orignal into a fragment
	our_ref.setScale(glm::vec3(SPLIT_SIZE));
	a.isFragment = true;
	...
	for (int i = 0; i < 3; i++)
	{
		...
		astriods.push_back(createAstroid(our_ref.getObjectId(), i + 1, true));
	}
}